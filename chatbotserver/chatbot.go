...
//returns result, images, tracks, alarmTime, error
func sampleProcessor(session Session, message string, uuid string) (string, string, string, string, error) {
	...
	if strings.Contains(message, "featured playlists") {
		...
		return reply, "", tracks, alarmTime, nil
